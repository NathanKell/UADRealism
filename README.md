...
A ship hull can have ai_beamdraughtlimits() in its param column. ai_beamdraughtlimits takes 4 values: new beamMin, new beamMax, new draughtMin, new draughtMax. If the value is 0 for one of them, the original value is kept. For example ai_beamdraughtlimits(-1,1,-2,2) would limit the AI to offets of +/-1% each in beam and +/-2% in draught. If you desire no offset, use a very small number like 0.001.
